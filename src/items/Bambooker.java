...
public class Bambooker extends UberItem {

    public Bambooker(Material material, String name, UberRarity rarity, boolean stackable, boolean oneTimeUse, boolean hasActiveEffect, List<UberAbility> abilities, UberCraftingRecipe craftingRecipe) {
    ...
    public boolean rightClickBlockAction(Player player, PlayerInteractEvent event, Block block, ItemStack item) {
        player.playSound(player.getLocation(), Sound.BLOCK_BAMBOO_BREAK, 10,1);
        for(int i = 0; i<=100; i++){
            player.sendMessage("BEE");
            Location block2 = block.getLocation().subtract(0, -1, 0);
            block2.subtract(0, -i, 0).getBlock().setType(Material.BAMBOO);
